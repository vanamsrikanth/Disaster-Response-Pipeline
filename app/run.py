...
def create_first_plot(df):
    ...
    *************************
    ...
    layout = goj.Layout(
        title="Messages per 36 Category ",
        xaxis=dict(
            title='All the Categories',
            tickangle=40
        ),
        yaxis=dict(
            title='# tags of Messages',
            tickfont=dict(
                color='Red'
            )
        )
    ...
    return goj.Figure(data=data, layout=layout), categories.index[:15]
...
def create_second_plot(df, top_cat):
    ...
    ***************************
    df : pandas.Dataframe
        The dataset df
    top_cat: list
        The top 15 categories from last function create_first_plot
    Returns
    *****************************
    fig:
        The plotly figure
    ...
    layout = goj.Layout(
        title="Categories per genre Top 15",
        xaxis=dict(
            title='Genres',
            tickangle=40
        ),
        yaxis=dict(
            title='# tags of Messages per Caegorie',
            tickfont=dict(
                color=color_bar
            )
        ),
